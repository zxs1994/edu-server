...
public interface AdminUserService {
    ...
    /**
     * 获得指定状态的用户们，并排除拥有指定角色编码的用户
     *
     * @param status 状态
     * @param excludeRoleCode 排除的角色编码（如 student）
     * @return 用户们
     */
    List<AdminUserDO> getUserListByStatusExcludeRole(Integer status, String excludeRoleCode);

    /**
     * 获得指定状态的用户们，且仅保留拥有指定角色编码之一的用户
     *
     * @param status 状态
     * @param includeRoleCodes 包含的角色编码列表（如 student、teacher）
     * @return 用户们
     */
    List<AdminUserDO> getUserListByStatusIncludeRoles(Integer status, Collection<String> includeRoleCodes);
