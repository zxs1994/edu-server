package cn.dh.edu.module.infra.service.codegen.inner;
...
import cn.dh.edu.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.dh.edu.framework.common.util.date.LocalDateTimeUtils;
...
import cn.dh.edu.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.dh.edu.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.dh.edu.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.dh.edu.module.infra.enums.codegen.CodegenSceneEnum;
import cn.dh.edu.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.dh.edu.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.dh.edu.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 dh-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 dh-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.dh.edu.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/dh-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/dh-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "dh-module-${table.moduleName}/" + // 顶级模块
                "dh-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "dh-module-${table.moduleName}/" + // 顶级模块
                "dh-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "dh-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "dh-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "dh-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "dh-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
