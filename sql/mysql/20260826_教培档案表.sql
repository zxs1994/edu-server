...
CREATE TABLE IF NOT EXISTS `edu_teacher` (
  ...
  `reward_standard` decimal(12, 2) NULL DEFAULT NULL COMMENT '报酬/奖励标准（金额；选项来自字典 edu_teacher_reward）',
...
-- 字典：报酬/奖励标准（value 为金额数字字符串，可 Number 转换）
INSERT INTO `system_dict_type` (`id`, `name`, `type`, `status`, `remark`, `creator`, `create_time`, `updater`, `update_time`, `deleted`, `deleted_time`)
...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `system_dict_data` WHERE `dict_type` = 'edu_teacher_reward' AND `value` = '300');

INSERT INTO `system_dict_data` (`id`, `sort`, `label`, `value`, `dict_type`, `status`, `color_type`, `css_class`, `remark`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
SELECT 22111, 2, '500元', '500', 'edu_teacher_reward', 0, 'processing', '', '报酬标准 500 元', '1', NOW(), '1', NOW(), b'0'
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `system_dict_data` WHERE `dict_type` = 'edu_teacher_reward' AND `value` = '500');

INSERT INTO `system_dict_data` (`id`, `sort`, `label`, `value`, `dict_type`, `status`, `color_type`, `css_class`, `remark`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
SELECT 22112, 3, '800元', '800', 'edu_teacher_reward', 0, 'success', '', '报酬标准 800 元', '1', NOW(), '1', NOW(), b'0'
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `system_dict_data` WHERE `dict_type` = 'edu_teacher_reward' AND `value` = '800');
