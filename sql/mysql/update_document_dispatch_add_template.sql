...
CREATE TABLE IF NOT EXISTS `oa_red_template` (
  ...
  `org_name` varchar(200) DEFAULT NULL COMMENT '机关/公司名称（红头大字）',
  `name_font_size` int NOT NULL DEFAULT 36 COMMENT '名称字号（红头大字字号）',
  `doc_number_prefix` varchar(50) DEFAULT NULL COMMENT '字号前缀（如：无办发）',
  `seal_image` varchar(500) DEFAULT NULL COMMENT '印章图片URL',
  `separator_style` varchar(50) NOT NULL DEFAULT 'single' COMMENT '分隔线样式（single=单线 double=双线）',
